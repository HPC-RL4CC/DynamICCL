...
/*
 * pluginInit: Initialize plugin with `context` and `LogFucntion` 
 *              for the given number of nodes and ranks.
 *              
 *   NOTE: CONTEXT is the inforamtion that will be shared with 
 *          `pluginGetCollInfo` function via NCCL Comm.
 */
...
/*
 * pluginGetCollInfo: Function called whenever a Task <CollFunc, Operation, Datatype> 
 *                    is created. 
 *                    Tuning is done by `collCostTable[a][p]` and `nChannels` values. 
 *  
 *  NOTE: Initialized CONTEXT can be used here to perform static tuning.
 *  
 *  TODO: Check if dynamic tuning can be done as well using func pointers inside the 
 *        `context` structure. :)
 */
...
/*
 * pluginDestroy: Destructor for plugin object
 */
...
#define PLUGIN_NAME "[#] NCCL Example.!.!"
